from data_drawer import draw_pie, draw_center_users
import os
import pandas as pd
import numpy as np
...
np.savetxt(os.path.join(SAVE_PATH, 'test6_3_3_cluster_centers.txt'),
           km_cluster_centers)
np.savetxt(os.path.join(SAVE_PATH, 'test6_3_3_labels.txt'), labels)
...
draw_pie(nums_cluster, labels=['DMTP ' + str(i) for i in range(1, k + 1)],
         filepath=SAVE_PATH+'test6_3_6_pie', formats=('svg', 'png', 'tif'))

draw_center_users(7, 8, center_users=np.array(user_mean), ymax=None, label1='DMTP', label2='DLTP',
                  filepath=SAVE_PATH+'test6_3_6_user', formats=('svg', 'png', 'tif'))
