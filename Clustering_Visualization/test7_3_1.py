...
for choice in ['LOW', 'BASE', 'HIGH']:
    Loads = pd.read_csv(BASIC_PATH+f'new_resident_WLPs_{choice}.csv')
    if WLPs is None:
        WLPs = Loads
    else:
        WLPs = pd.concat(
...
Dates = WLPs['Date'].to_list()
CONS_NOs = []
for date in Dates:
    CONS_NOs.append(date.split('_')[1])
WLPs.drop(['Date'], axis=1, inplace=True)
WLPs = WLPs.to_numpy()
...
labels = np.loadtxt(os.path.join(
    LABEL_PATH, 'test7_2_1_labels_'+str(K)+'.txt'))
